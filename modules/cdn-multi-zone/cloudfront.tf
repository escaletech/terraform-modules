...
resource "aws_cloudfront_distribution" "main" {
  ...
  dynamic "origin" {
    ...
    content {
      ...
      origin_id   = "Custom-${origin.value["host"]}"
  ...
  dynamic "ordered_cache_behavior" {
    ...
    content {
      ...
      target_origin_id       = "Custom-${behavior.value.target_origin_id}"
      compress               = lookup(behavior.value, "compress", null)
      viewer_protocol_policy = behavior.value.viewer_protocol_policy

      cache_policy_id          = lookup(behavior.value, "cache_policy_id", null)
      origin_request_policy_id = lookup(behavior.value, "origin_request_policy_id", null)

      dynamic "lambda_function_association" {
        iterator = lambda
        for_each = lookup(behavior.value, "lambda_function_association", [])
        content {
      ...
      dynamic "function_association" {
        iterator = cffunction
        for_each = lookup(behavior.value, "function_association", [])
        content {
